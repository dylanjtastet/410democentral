...
export const fetchCategoryArray = () => {
	return dispatch => {
		...
		let categoryURL = new URL(process.env.REACT_APP_BASEURL+"allcategories");
		fetch(categoryURL, {
			credentials: "include"
		})
		.then(res => {
...
export const fetchCategoryTree = () => {
	return dispatch => {
		...
		let dirURL = new URL(process.env.REACT_APP_BASEURL+"dir");
		fetch(dirURL, {
			credentials: "include"
		})
		.then(res => {
...
export const createCategory = (name, parent, group) => {
	return dispatch => {
		...
		let categoryURL = new URL(process.env.REACT_APP_BASEURL+"category");
