...
export const fetchGroups = (resetActiveGroup = true) => {
	return async dispatch => {
		...
		let allgroupURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"allgroups");
		try {
			...
			for (let i = 0; i < data.length; i++) {
				if (data[i].isInstructor) {
					let groupURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"group");
...
export const fetchGroupNames = async () => {
	let groupnameURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"allgroupnames")
	try {	
...
export const removeMemberFromGroup = (name, member = null) => {
	return dispatch => {
		...
		let removeURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"removefrom/" + name);
		if (member !== null) {
...
export const addInstructorToGroup = (groupName, username) => {
	return dispatch => {
		let addInstructorURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"addinstructor");
...
export const removeInstructorFromGroup = (groupName, username) => {
	return dispatch => {
		let rmInstructorURL = new URL(process.env.REACT_APP_BASEURL + ":" + process.env.REACT_APP_BACKEND_PORT + "/"+"removeinstructor");
